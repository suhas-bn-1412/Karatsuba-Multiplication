...
{
	printf("call to compute_uh with str: %s  sz: %d\n", str,sz);
...
{
	printf("call to compute_lh with str: %s  sz: %d\n", str,sz);
	int len = strlen(str);
...
{
	printf("call to str_mul with a: %s  b: %s\n", a,b);
	int x = atoi(a);
	int y = atoi(b);
	int ans = x*y;
...
{
	printf("call to str_add with a: %s  b: %s\n", a,b);
	int len_a = strlen(a);
	int len_b = strlen(b);
...
{
	printf("call to str_sub with a: %s  b: %s\n", a,b);
	int len_a = strlen(a);
	int len_b = strlen(b);
...
char* karatsuba(char* x, char* y)
{
	...
	else
	{
		...
		if(strcmp(uh_x,zero) || strcmp(uh_y,zero))
			a = zero;
		else
			a = karatsuba(uh_x,uh_y);

		if(strcmp(lh_x,zero) || strcmp(lh_y,zero))
			d = zero;
		else
